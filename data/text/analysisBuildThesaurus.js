...
wordsx.forEach( word => {
	let url = `https://www.dictionaryapi.com/api/v3/references/thesaurus/json/${word.text}?key=${keys.thesaurus}`;
	word.def = [];
	...
	https.get(url, res => {
		...
		res.on('end', () => {
			...
			try {
				...
				// let entry = ( JSON.parse(responses.join()) ).filter(e =>  e.hwi.hw.indexOf(' ') === -1);
				word.def.push(...entry);
				// word.defstr=JSON.stringify(entry);
				// console.log("def :: " + JSON.stringify(word.def));
				console.log("done :: " + word.text);
				console.log("completed_requests :: " + completed_requests + " wordsx.length :: " + testn);
				// fs.writeFile('thesaurusTest/'+ word.text +'.js', JSON.stringify(entry,null,"\t"), 'utf8', e => {console.log("done :: " + word.text)});

		  	// if (completed_requests === wordsx.length - 1) {
		  	if (!(completed_requests < testn - 1)) {
		  		console.log("true");
		  		fs.writeFile('analysisOutputThesaurus'+Date.now()+'.js', JSON.stringify(wordsx, null, "\t"), 'utf8', e => {console.log("done")});
		  	}
		  } catch(e) {}
